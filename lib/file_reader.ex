defmodule FileReader do
  ...
  def lines_lengths!(path), do: get_lines!(path) |> Stream.map(&(String.length(&1))) |> Enum.map(&(&1))
  ...
  def words_per_line!(path), do: get_lines!(path) |> Stream.map(&(String.split(&1))) |> Enum.map(&(length(&1)))
